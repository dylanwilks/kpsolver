...
use kpsolver::{
    BinaryItem, Item, items, items_binary,
    BinaryKnapsack, Knapsack, knapsacks, knapsacks_binary,
    BinarySolver, BoundedSolver,
};
...
) -> (<S1 as BinarySolver<T1, N>>::Output, <S2 as BinarySolver<T2, N>>::Output)
where
    ...
    S2: BinarySolver<T2, N>,
{
    ...
    let solution_1 = problem_items.clone().to_generic::<T1>().insert_into(
        problem_knapsacks.clone().to_generic::<T1>()).using(solver);

    let solution_2 = problem_items.to_generic::<T2>().insert_into(
        problem_knapsacks.to_generic::<T2>()).using(optimal_solver);
...
(
    ...
    capacities_min: [u32; N],
    capacities_max: [u32; N],
) -> (<S1 as BoundedSolver<T1, N>>::Output, <S2 as BoundedSolver<T2, N>>::Output)
where
    T1: CompatibleProblemType + From<u32>,
    T2: CompatibleProblemType + From<u32>,
